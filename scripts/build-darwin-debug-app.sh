...
# Fumie does not ship the Copilot CLI harness. `sessionsAllowedAgentHostProviders`
# in product.json omits `copilotcli`, so neither agentHostMain nor
# agentHostServerMain ever registers CopilotAgent and nothing reaches
# `resolveCopilotCliPath`. The platform CLI package it would have spawned is
# 148MB of dead weight, so drop it from the staged runtime. Upstream packaging
# still unpacks it (build/lib/copilot.ts re-adds it after .moduleignore strips
# every @github/copilot-* package), which leaves its entries in the ASAR header
# marked `unpacked`; that is inert because no code path resolves them once the
# harness is gone.
COPILOT_CLI_PACKAGES="$RUNTIME_APP/Contents/Resources/app/node_modules.asar.unpacked/@github"
if [[ -d "$COPILOT_CLI_PACKAGES" ]]; then
	for copilot_cli_package in "$COPILOT_CLI_PACKAGES"/copilot-*; do
		[[ -e "$copilot_cli_package" ]] || continue
		echo "[fumie-debug] dropping Copilot CLI package $(basename "$copilot_cli_package") ($(du -sm "$copilot_cli_package" | cut -f1)MB); the harness is disabled in product.json" >&2
		rm -rf "$copilot_cli_package"
	done
	rmdir "$COPILOT_CLI_PACKAGES" 2>/dev/null || true
fi

# Copilot ships its complete build-time dependency tree (380MB) even though
# esbuild inlined nearly all of it into dist/. Keep the closure of what the
# packaged bundle still requires at runtime, plus the debug helpers staged above,
# and drop the rest.
"$NODE_BINARY" - "$RUNTIME_EXTENSIONS/copilot" <<'NODE'
const fs = require('fs');
...
// Fumie removed the Copilot CLI harness, so the 121MB @github/copilot SDK the
// bundle reaches through `await import('@github/copilot/sdk')` goes with it.
// That import is the extension's only use of the package and it is already
// wrapped in try/catch (CopilotCLISDK.getPackage), so its absence is a logged
// failure of a feature Fumie does not offer, not a broken activation. Listed
// here rather than left to the closure walk so the before/after load probe
// below treats it as intentionally removed instead of a regression.
const droppedPackages = new Set(['@github/copilot']);
...
for (const entry of fs.readdirSync(distDir, { withFileTypes: true, recursive: true })) {
	...
	for (const pattern of specifierPatterns) {
		for (const [, specifier] of source.matchAll(pattern)) {
			...
			if (!builtins.has(name) && name !== 'vscode' && !droppedPackages.has(name)) {
...
const loads = names => Object.fromEntries(names.map(name => {
	...
	const status = cp.spawnSync(process.execPath, ['--input-type=module', '-e', probe], {
		cwd: extensionDir,
		stdio: 'ignore',
	}).status;
...
// `droppedPackages` wins over the closure walk: another kept package declaring
// @github/copilot as a dependency must not drag the SDK back in.
const keep = new Set([...installed].filter(name => !droppedPackages.has(name) && keepDirectories.has(path.join(modulesDir, name))));
...
const droppedOnPurpose = [...droppedPackages].filter(name => installed.has(name));
if (droppedOnPurpose.length) {
	console.error(`[fumie-debug] copilot node_modules: dropped ${droppedOnPurpose.join(', ')} with the Copilot CLI harness`);
}
console.error(`[fumie-debug] copilot node_modules: kept ${keep.size} of ${installed.size} packages (runtime roots: ${presentRoots.sort().join(', ')})`);
NODE
