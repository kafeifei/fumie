...
Only **Existing Sessions** share a persisted Editor/Details visibility profile. Its initial state is **Detail only**, keeping chat primary. After restore, an Existing Session also collapses redundant editor content when every open tab is managed and the active tab is Empty Files (or Changes with no actual changes); a real file or non-empty Changes keeps or reveals Editor. A New Session does not apply or capture that profile; on entry it hides Editor once only when the restored editor set contains no input other than Empty Files. Submit seeds the Existing profile. The active editor selects the detail content: every diff editor selects Changes and every file editor selects Files.
