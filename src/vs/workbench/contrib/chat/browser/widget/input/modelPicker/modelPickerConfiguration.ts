...
type ChatThinkingEffortChangeClassification = {
	owner: 'lramos15';
	comment: 'Reporting when a model configuration value (e.g. thinking effort, service tier, or the Auto routing tier) is changed';
	model: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The model the configuration was changed for' };
	property: { classification: 'SystemMetaData'; purpose: 'FeatureInsight'; comment: 'The first-party configuration property that was changed (reasoningEffort, serviceTier, or tier for the Auto model); "unknown" for third-party providers, which choose their own keys' };
...
export class ModelPickerConfiguration {
	...
	renderButton(button: HTMLElement, compact: boolean, noModelsAvailable: boolean): void {
		...
		const performanceConfig = this._getConfigProperty('performance');
		const tokensConfig = this._getConfigProperty('tokens');
		if (compact || !model || noModelsAvailable || (!effortConfig && !performanceConfig && !tokensConfig)) {
		...
		// Keep the default service quiet in the at-rest summary. Non-default tiers
		// are explicit because they can affect speed and usage.
		if (performanceConfig && performanceConfig.value !== undefined && performanceConfig.value !== performanceConfig.schema.default) {
			const enumIndex = performanceConfig.schema.enum?.indexOf(performanceConfig.value) ?? -1;
			const performanceLabel = enumIndex >= 0 && performanceConfig.schema.enumItemLabels?.[enumIndex]
				? performanceConfig.schema.enumItemLabels[enumIndex]
				: String(performanceConfig.value);
			labelParts.push(performanceLabel);
			ariaParts.push(performanceConfig.schema.title
				? localize('chat.modelPicker.performanceAriaLabel', "{0}: {1}", performanceConfig.schema.title, performanceLabel)
				: localize('chat.modelPicker.speedAriaLabel', "Speed: {0}", performanceLabel));
		}
		...
		if (!labelParts.length) {
			...
			const fallbackLabel = effortConfig?.schema.title ?? performanceConfig?.schema.title ?? tokensConfig?.schema.title ?? localize('chat.modelPicker.configureLabel', "Configure");
	...
	private _buildItems(): IActionListItem<IActionWidgetDropdownAction>[] {
		...
		appendConfigSection(
			'performance',
			localize('chat.performance.header', "Speed"),
			(value, enumLabel) => enumLabel ?? String(value),
			(value, previousValue, key) => this._telemetryService.publicLog2<ChatThinkingEffortChangeEvent, ChatThinkingEffortChangeClassification>('chat.thinkingEffortChange', {
				model: model.metadata.vendor === 'copilot' ? new TelemetryTrustedValue(modelIdentifier) : 'unknown',
				property: model.metadata.vendor === 'copilot' ? key : 'unknown',
