...
|---|---|---|
| `respondToPermissionRequest(requestId, approved: boolean, selectedOptionId?: string)` | tool-permission gates | the deferred parked inside `Options.canUseTool` — Claude ignores the picked option, since `canUseTool` takes one allow/deny answer |
...
### M10 — Steering and truncation: `setPendingMessages`, turn promotion, `truncateSession`

| Direction | Client → Host (write) for steering / truncation, Host → Client (`ChatTurnComplete` / `ChatTurnStarted`) for promotion |
|---|---|
