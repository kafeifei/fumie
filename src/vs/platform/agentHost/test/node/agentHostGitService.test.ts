...
suite('AgentHostGitService', () => {
	...
	suite('collapseWorktreeIncludePaths', () => {
		/**
		 * Collapses an ignored-file listing, where a `+` prefix marks the files
		 * the configured globs matched, into the directories that can be copied
		 * as one recursive unit (with the number of matched files each covers)
		 * plus the matched files left to copy individually.
		 */
		function collapse(wholeDirectories: readonly string[], listing: readonly string[]): { collapsed: [string, number][]; standalone: string[] } {
